...
go-run-autotests:
	...
	metricstest-darwin-arm64 -test.v -test.run=^TestIteration${TEST_NUM}$$ -agent-binary-path=cmd/agent/agent -binary-path=cmd/server/server -server-port=12345 -source-path=.